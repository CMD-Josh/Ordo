 ...
 */
package org.ordo.project.dao.jaxbimpl;
...
import org.ordo.project.model.EntityList;
...
public class EntityDAOJaxbImpl implements EntityDAO {
    ...
    private static final String CONTEXT_PATH = "org.ordo.project.model";
    ...
    public List<Entity> retrieveMatchingEntities(Entity entityTemplate) {
        ...
        for (Entity entity : entityList.getEntities()) {
            ...
            if (entityTemplate.getTimeStart() != null) {
                if (!entityTemplate.getTimeStart().equals(entity.getTimeStart())) {
                    match = false;
                }
            };
            if (entityTemplate.getTimeEnd()!= null) {
                if (!entityTemplate.getTimeEnd().equals(entity.getTimeEnd())) {
                    match = false;
                }
            };
            if (entityTemplate.getPrice()!= null) {
                if (!entityTemplate.getPrice().equals(entity.getPrice())) {
                    match = false;
                }
            };
            if (match) {
        ...
        synchronized (Lock) {
            for (Entity en : entityList.getEntities()) {
                if (entityTemplate.getId().equals(en.getId())) {
                    ...
                    if (entityTemplate.getTimeStart() != null) {
                        en.setTimeStart(entityTemplate.getTimeStart());
                        changedfield = true;
                    }
                    if (entityTemplate.getTimeEnd() != null) {
                        en.setTimeEnd(entityTemplate.getTimeEnd());
                        changedfield = true;
                    }
                    if (entityTemplate.getPrice() != null) {
                        en.setPrice(entityTemplate.getPrice());
                        changedfield = true;
                    }
